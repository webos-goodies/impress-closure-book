...
tinyword.RightPane.prototype.enterDocument = function() {
  ...
  // リッチテキストエディタの作成
  var dom = this.getDomHelper(), el = this.getElement();
  var contentEls = dom.getElementsByTagNameAndClass(
    'div', tinyword.RightPane.CONTENT_CLASS_NAME_, el);
  this.editor_ = new tinyword.Editor(this, contentEls[0], dom);
};

// ドキュメントから削除された時の処理
tinyword.RightPane.prototype.exitDocument = function() {
...
tinyword.RightPane.prototype.onSelectTab = function(e) {
  ...
  // プレビューの更新
  if(this.tabBar_.getSelectedTab().getId() == 'preview-tab') {
    els[index].innerHTML = this.editor_.getCleanContents();
  }
};
...
tinyword.RightPane.prototype.resize = function(size) {
  ...
  this.editor_.resize(new goog.math.Size(size.width - 2, contentSize));
};

// リッチテキストエディタへのアクセサメソッド
tinyword.RightPane.prototype.getEditor = function() {
  return this.editor_;
};
