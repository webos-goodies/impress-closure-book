...
tinyword.Editor.prototype.resize = function(size) {
  ...
  var editorSize  = Math.max(size.height - toolbarSize, 0); goog.style.setStyle(
...
tinyword.Editor.prototype.loadFile = function(path) {
  if(path != this.filePath_) {
    ...
    if(node) {
      this.loadingId_ = (this.loadingId_ || 0) + 1;
      tinyword.App.getInstance().sendRequest(
        'file', { 'id': node.get()|0 },
        goog.bind(this.loadFileComplete_, this, path, this.loadingId_), 'GET');
    }
  }
};

tinyword.Editor.prototype.loadFileComplete_ = function(path, id, body) {
  if(id == this.loadingId_) {
    this.setHtml(false, body['content'], true);
