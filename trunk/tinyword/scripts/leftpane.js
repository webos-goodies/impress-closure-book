  ...
  if(treeNode && treeNode.getFileType() == 'folder') {
	...
	if(window['user_id']) {
      tinyword.App.getInstance().sendRequest(
		...
		goog.bind(this.onNewFolderComplete_, this, parentNode),
		'POST', { '#text': '新規フォルダ' });
	} else {
...
tinyword.LeftPane.prototype.onNewFolderComplete_ = function(parentNode, body) {
  var entryNode = parentNode && parentNode.getChildNode('entry', true);
  ...
  if(treeNode && !treeNode.isRootNode()) {
    ...
    if(name = window.prompt('新しい名前を指定してください。', name)) {
	  if(window['user_id']) {
		tinyword.App.getInstance().sendRequest(
          'tree', { 'path': path },
          goog.bind(this.onRenameComplete_, this, node, name),
          'PUT', { '#text': name });
	  } else {
...
tinyword.LeftPane.prototype.onRenameComplete_ = function(node, name, body) {
  node.setChildNode('#text', name);
};
...
tinyword.LeftPane.prototype.onDelete_ = function(e) {
  ...
  if(treeNode && !treeNode.hasChildren() && !treeNode.isRootNode()) {
	if(window['user_id']) {
      ...
      tinyword.App.getInstance().sendRequest(
		'tree', { 'path': path },
		goog.bind(this.onDeleteComplete_, this, path), 'DELETE');
	} else {
...
tinyword.LeftPane.prototype.onDeleteComplete_ = function(path, body) {
...
// ツリーコントロールへのアクセサメソッド
tinyword.LeftPane.prototype.getTree = function() {
  return this.treeControl_;
};
